...
- Run `python preprocess.py` once (if `./preprocessed/` is not already available), then run `python MS_Per-Lead_Attn.py` end-to-end. The script handles data loading, training, checkpoint selection, threshold tuning, and evaluation automatically (CUDA, Apple MPS, or CPU via `device.py`).
