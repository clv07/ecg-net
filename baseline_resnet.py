...
Baseline 4: ResNet1D
=====================
A 1D ResNet baseline modeled on the architecture from Strodthoff et al.
(2021) "Deep Learning for ECG Analysis: Benchmarks and Insights from
PTB-XL", which reported ResNet-style models as the strongest CNN
baselines on PTB-XL superclass classification.

This implementation follows the xresnet1d101 design philosophy:
    - Initial "stem" block with two conv layers
    - Four stages of residual blocks (basic blocks, BatchNorm, ReLU)
    - Progressive downsampling via stride-2 convs
    - Global average pooling → linear classifier

Architecture (channels × spatial):
    Stem:       (12, 1000)  →  (64, 250)
    Stage 1:    (64, 250)   →  (64, 250)     2 blocks
    Stage 2:    (64, 250)   →  (128, 125)    2 blocks
    Stage 3:    (128, 125)  →  (256, 63)     2 blocks
    Stage 4:    (256, 63)   →  (512, 32)     2 blocks
    Pool:       (512, 32)   →  (512,)
...
class BasicBlock1D(nn.Module):
    """
    Standard 1D residual block (no bottleneck):
        Conv → BN → ReLU → Conv → BN → (+ skip) → ReLU
    The first conv handles any downsampling (stride > 1).
    """
    expansion = 1
...
class ResNet1D(nn.Module):
    ...
    By default uses [2, 2, 2, 2] blocks per stage (~ResNet-18 depth),
    which gives a strong CNN baseline without the training instability
    of deeper variants on small datasets like PTB-XL.
    """
    def __init__(self,
                 num_leads=12,
                 num_classes=5,
        ...
        # --- Stem: large kernel, aggressive downsampling ---
        self.stem = nn.Sequential(
            nn.Conv1d(num_leads, stem_channels,
        ...
        for stage_idx, (out_ch, n_blocks) in enumerate(zip(stage_channels,
                                                            blocks_per_stage)):
            # First block of each stage (except stage 0) downsamples
...
def main():
    ...
    print("\nInitializing ResNet1D...")
    model = ResNet1D(num_classes=len(SUPERCLASSES)).to(DEVICE)
    ...
    print("\n" + "=" * 60)
    print("FINAL TEST RESULTS — ResNet1D")
    print("=" * 60)
